...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      // phi is referenced counter-clockwise from the x-axis.
  ...
  /**
   * TODO: Update the state transition matrix F according to the new elapsed time. (Time is measured in seconds.)
   * TODO: Update the process noise covariance matrix.
   * Use noise_ax = 9 and noise_ay = 9 for your Q matrix.
   */

  // dt - expressed in seconds, divide the result by 10^6 to transform it from microseconds to seconds.
  ...
  float py = ekf_.x_(1);
  float s = px * px + py * py;
  // skip the update step if s is close to 0
  if (s > 0.0001){
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      // TODO: Radar updates
